<?php
    ...
    class Message extends Model {
        ...
        public function validate(){
            ...
            public function find($send_user_id,$receive_user_id){
                    try {
                        $pdo = self::get_connection();
                ...
                //メッセージ処理で注目するユーザーに紐付いたユーザー一覧を取得
                public static function message_relation($id){
            
                try {
                    $pdo = self::get_connection();
                    $stmt = $pdo -> prepare("select messages.send_user_id,messages.receive_user_id from messages Where send_user_id=id: or receive_user_id=:id");//変数値を保持しているのでprepare
                    // バインド処理
                    ...
                    $stmt->setFetchMode(PDO::FETCH_CLASS|PDO::FETCH_PROPS_LATE, 'Message');
                    // Userクラスのインスタンスを返す
                    $messages_relations = $stmt->fetchAll();  
                    self::close_connection($pdo, $stmp);
