<?php
 ...
 class User extends Model
 {
         ...
         try {
             $pdo = self::get_connection();
              ...
              if ($this->id === null) {
                 ...
                 $stmt = $pdo->prepare('UPDATE users SET name=:name,password=:password,account=:account WHERE id=:id');//変数値を保持しているのでprepare
     ...
     //入力チェックバリデーション
     public function edit_validation($account)
     {
         try {
             $errors = array();
             $pdo = self::get_connection();
     ...
     //idから対象のUserオブジェクトを取得するメソッド
     public static function find_user_info($id)
     {
         try {
             $pdo = self::get_connection();
             $stmt = $pdo->prepare('SELECT * FROM users WHERE id=:id');//変数値を保持しているのでprepare
            // バインド処理
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            // 実行
            $stmt->execute();
